...
class TabQAgent(object):
    ...
    def __init__(self, actions=[], epsilon=0.1, alpha=0.1, gamma=1.0, debug=False, canvas=None,canvas2=None, root=None):
    ...
    def act(self, world_state, agent_host, current_r,my_mission ):
        ...
        # if int(obs[u'XPos'])==3 and int(obs[u'ZPos'])==2:
        #     my_mission.rewardForReachingPosition(3,45,2,-100,0.0)
        # if int(obs[u'XPos'])==1 and int(obs[u'ZPos'])==4:
        #     my_mission.rewardForReachingPosition(1,45,4,-100,0.0)
        
        print ("current position is",int(obs[u'XPos']) )
        self.logger.debug("State: %s (x = %.2f, z = %.2f)" % (current_s, float(obs[u'XPos']), float(obs[u'ZPos'])))
        if current_s not in self.q_table:
            self.q_table[current_s] = ([0] * len(self.actions))
        if current_s not in self.q_table2:
            self.q_table2[current_s] = ([0] * len(self.actions)) 

        # update Q values
        if self.training and self.prev_s is not None and self.prev_a is not None:
            old_q = self.q_table[self.prev_s][self.prev_a]
            old_q2= self.q_table2[self.prev_s][self.prev_a]
            if self.updateA==True:
                self.q_table[self.prev_s][self.prev_a] = old_q + self.alpha * (current_r
                    + self.gamma * max(self.q_table2[current_s]) - old_q)
            else:
                self.q_table2[self.prev_s][self.prev_a] = old_q2 + self.alpha * (current_r
                    + self.gamma * max(self.q_table[current_s]) - old_q2)
        ...
        if rnd < self.epsilon:
            ...
            if whichTable>0.5:
                ...
                self.updateA=False
                m2= max(self.q_table2[current_s])
                self.logger.debug("Current values: %s" % ",".join(str(x) for x in self.q_table2[current_s]))
                l = list()
                for x in range(0, len(self.actions)):
                    if self.q_table2[current_s][x] == m2:
                        l.append(x)
                y = random.randint(0, len(l)-1)
                a = l[y]
                self.logger.info("Taking q action: %s" % self.actions[a])
    ...
    def run(self, agent_host,my_mission):
        ...
        # take first action
        total_reward += self.act(world_state,agent_host,current_r,my_mission)
        ...
        if self.training and self.prev_s is not None and self.prev_a is not None:
            ...
            old_q2 = self.q_table2[self.prev_s][self.prev_a]
            self.q_table2[self.prev_s][self.prev_a] = old_q2 + self.alpha * ( current_r - old_q2 )
    ...
    def drawQ( self, curr_x=None, curr_y=None ):
        if self.canvas is None or self.root is None or canvas2 is None:
            return
        ...
        for x in range(world_x):
            for y in range(world_y):
                ...
                self.canvas2.create_rectangle( (world_x-1-x)*scale, (world_y-1-y)*scale, (world_x-1-x+1)*scale, (world_y-1-y+1)*scale, outline="#fff", fill="#000")
                for action in range(4):
                    ...
                    if not s in self.q_table2:
                        continue
                    value = self.q_table2[s][action]
                    color = int( 255 * ( value - min_value ) / ( max_value - min_value )) # map value to 0-255
                    color = max( min( color, 255 ), 0 ) # ensure within [0,255]
                    color_string = '#%02x%02x%02x' % (255-color, color, 0)
                    self.canvas2.create_oval( (world_x - 1 - x + action_positions[action][0] - action_radius ) *scale,
                                             (world_y - 1 - y + action_positions[action][1] - action_radius ) *scale,
                                             (world_x - 1 - x + action_positions[action][0] + action_radius ) *scale,
                                             (world_y - 1 - y + action_positions[action][1] + action_radius ) *scale, 
                                             outline=color_string, fill=color_string )
        if curr_x is not None and curr_y is not None:
            ...
            self.canvas2.create_oval( (world_x - 1 - curr_x + 0.5 - curr_radius ) * scale, 
                                     (world_y - 1 - curr_y + 0.5 - curr_radius ) * scale, 
                                     (world_x - 1 - curr_x + 0.5 + curr_radius ) * scale, 
                                     (world_y - 1 - curr_y + 0.5 + curr_radius ) * scale, 
                                     outline="#fff", fill="#fff" )
...
canvas.grid()
canvas2 = tk.Canvas(root, width=world_x*scale, height=world_y*scale, borderwidth=0, highlightthickness=0, bg="black")
canvas2.grid()
root.update()
...
for imap in range(num_maps):
    ...
    #actionSet = ["movenorth 1", "movesouth 1", "movewest 1", "moveeast 1", "jump 1"]
    ...
    for i in range(num_repeats):
        # my_mission.rewardForReachingPosition(3,45,2,1,0.0)
        # my_mission.rewardForReachingPosition(1,45,4,100,0.0)
        ...
        cumulative_reward = agent.run(agent_host,my_mission)
